...
Google Visualization bietet viele Möglichkeiten, Diagramme und Tabellen in GWT zu erstellen. Dabei werden die Diagramme in Javascript geschrieben. Allerdings bietet Google auch ein Jar-Archiv für GWT-Projekte an, sodass an dieser Stelle der Programmierer Java-Code schreiben kann. \\
Um ein Diagramm zu erstellen, muss der Entwickler eine Datentabelle (DataTable) erstellen und dabei die Formate der Eingabedaten festlegen. Diese Daten werden dem gewünschten Diagramm übergeben und es wird auf der Oberfläche dargestellt. Alles weitere wird mitgeliefert wie zum Beispiel die „Clickevents“, oder die dynamische Größenanpassung des Diagramms an die Menge der Daten.\\
...
\subsection{Daten-Bereitstellung}Die Daten-Bereitstellung für das Frontend geschieht mit GWT-RPC. Eine genauere Erklärung findet sich in Abschnitt \ref{ssec:aufbau}. Die bereitgestellten Methoden dienen der Abfrage der Datenbank und lassen außerdem Manipulationen zu. Hier wird starker Gebrauch von den DataAccessObjects gemacht. \todo{Beispiel-Code?}
